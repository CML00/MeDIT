...
from MeDIT.ArrayProcess  import ExtractPatch, XY2Index, Crop2DImage, DetectRegionBlurry
...
def testDetect():
    ...
    model_folder_path = r'c:\MyCode\MPApp\DPmodel\CSPCaProstateDetection'
    pca_detect = CST2AdcDwiProstateRoiDetect()
    pca_detect.LoadModel(model_folder_path)

    from MeDIT.SaveAndLoad import LoadNiiData
    t2_image, _, t2_data = LoadNiiData(r'z:\Data\CS_ProstateCancer_Detect_multicenter\ToTest\Chen Wan Jie\004_t2_fse_tra_Resize.nii', dtype=np.float32, is_show_info=True)
    dwi_image, _, dwi_data = LoadNiiData(r'z:\Data\CS_ProstateCancer_Detect_multicenter\ToTest\Chen Wan Jie\007_epi_dwi_tra_CBSF5_NL3_b750_Reg_Resize.nii', dtype=np.float32, is_show_info=True)
    adc_image, _, adc_data = LoadNiiData(r'z:\Data\CS_ProstateCancer_Detect_multicenter\ToTest\Chen Wan Jie\010_epi_dwi_tra_CBSF5_NL3_ADC_Reg_Resize.nii', dtype=np.float32, is_show_info=True)
    prostate_roi_image, _, prostae_roi_data = LoadNiiData(r'z:\Data\CS_ProstateCancer_Detect_multicenter\ToTest\Chen Wan Jie\prostate_roi.nii.gz', dtype=np.uint8)
    ...
    show_data = np.concatenate((Normalize01(t2_data), Normalize01(dwi_data), Normalize01(adc_data)), axis=1)
    show_roi = np.concatenate((prodict_data, prodict_data, prodict_data), axis=1)
    show_prostate_roi = np.concatenate((prostae_roi_data, prostae_roi_data, prostae_roi_data), axis=1)
